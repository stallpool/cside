...
const i_env = {
   debug: !!process.env.CSIDE_DEBUG,
   server: {
      host: process.env.CSIDE_HOST || '127.0.0.1',
      port: parseInt(process.env.CSIDE_PORT || '8080'),
      staticDir: process.env.CSIDE_STATIC_DIR?i_path.resolve(process.env.CSIDE_STATIC_DIR):null,
      httpsCADir: process.env.CSIDE_HTTPS_CA_DIR?i_path.resolve(process.env.CSIDE_HTTPS_CA_DIR):null,
   },
};
...
const server = createServer({
   ...
   admin: require('./api_admin').api,
   profile: require('./api_config').api,
   check: require('./session').check,
   userpass: require('./auth_userpass').api,
   gitlab: require('./auth_gitlabat').api,
   sso: require('./auth_sso').api,
});
server.listen(i_env.server.port, i_env.server.host, () => {
   console.log(`CSIDE SERVER is listening at ${i_env.server.host}:${i_env.server.port}`);
});
