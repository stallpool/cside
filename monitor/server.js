...
const i_env = {
   ...
   server: {
      ...
      port: parseInt(process.env.TINY_PORT || '29001'),
...
const server = createServer({
   ...
   vscode: require('./vscode').api,
});
server.listen(i_env.server.port, i_env.server.host, () => {
   console.log(`CSIDE instance monitor SERVER is listening at ${i_env.server.host}:${i_env.server.port}`);
});
